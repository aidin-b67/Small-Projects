...
const validateCred = (arr) => {
  ...
  for (let i = arr.length - 1; i >= 0; i--) {
    let digit = arr[i];
    // double the every second digit form right
...
const idInvalidCardCompanies =  (invalidArrays) => {
  let companies = []
  for (let card of invalidArrays){
    switch(card[0]) {
      case 3:
      // Double check the companies to make sure they are not doubled
      ...
      case 6:
        ...
        break;
      default:
        if (!companies.includes('Unknown')) companies.push('Comapny not found');
        break;      
    } 
  }
  return companies;
...
// Check the companies which have invalid card console log(idInvalidCardCompanies(findInvalidCards(batch))); 
